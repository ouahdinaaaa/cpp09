...
/*   Updated: 2024/01/04 20:48:22 by ayael-ou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Pmerge::Pmerge(int size, char **argv) : _tab2(size + 1), _tab(size + 1)
{
    ...
    std::cout << "\nTime to process a range of " << size - 1 << " element with std::Vector<int> : " << c_vec/static_cast<double>(CLOCKS_PER_SEC / 1000) << "seconde" << std::endl;
    std::cout << "Time to process a range of " << size - 1 << " element with std::List<int>   : " << c_start/static_cast<double>(CLOCKS_PER_SEC / 1000) << "seconde" << std::endl;
}
